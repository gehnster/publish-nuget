...
class Action {
    ...
    _tagCommit(version) {
        ...
        process.stdout.write(`VERSION::${TAG} >> $GITHUB_OUTPUT` + os.EOL)
    }
    ...
    _pushPackage(version, name) {
        ...
        process.stdout.write(`PACKAGE_NAME=${packageFilename} >> $GITHUB_OUTPUT` + os.EOL)
        process.stdout.write(`PACKAGE_PATH=${path.resolve(packageFilename)} >> $GITHUB_OUTPUT` + os.EOL)

        if (symbolsFilename) {
            process.stdout.write(`SYMBOLS_PACKAGE_NAME=${symbolsFilename} >> $GITHUB_OUTPUT` + os.EOL)
            process.stdout.write(`SYMBOLS_PACKAGE_PATH=${path.resolve(symbolsFilename)} >> $GITHUB_OUTPUT` + os.EOL)
        }
